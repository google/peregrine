...
class TcpPspManagerTest : public TestWithParam<SocketTestParam> {
 protected:
  TcpPspManagerTest()
      : cfg_(GetParam()),
        ...
        psp_token_xchg_([this](const PspToken& self_token, const Endpoint& peer,
          ...
          return TcpPspManager::ExchangePspTokens(*this->mgr_, self_token,
                                                  peer);
        }) {
    CHECK(self_.IsValid());
  ...
  ~TcpPspManagerTest() override { psp::TestOnly_SetPspTcpSyscalls(nullptr); }
 ...
 protected:
  ...
  TcpPspManager::PspTokenExchange psp_token_xchg_;
};

INSTANTIATE_TEST_SUITE_P(, TcpPspManagerTest,
                         Combine(/*family=*/Values(AF_INET, AF_INET6),
                                 /*gen_blocking=*/Values(true, false)),
                         ToString);

TEST_P(TcpPspManagerTest, HandlePspTokenExchange) {
  ...
  const auto self_token =
      TcpPspManager::ExchangePspTokens(*mgr_, peer_token, self_target);
  ASSERT_TRUE(self_token.ok());
  EXPECT_TRUE(self_token->IsValid());

  const Endpoint unknown_target = Endpoint::Create("127.0.0.1:9999");
  const auto status =
      TcpPspManager::ExchangePspTokens(*mgr_, peer_token, unknown_target);
  EXPECT_THAT(status, StatusIs(kNotFound));
}

TEST_P(TcpPspManagerTest, AcceptBeforeConnect) {
  if (!psp::IsPspSupported()) {
    GTEST_SKIP() << "psp not supported";
  }
  ...
  util::Thread tc([&]() {
    ...
    for (const NicInfo& ni : peers_) {
      ...
      std::unique_ptr<TcpSocket> socket =
          TcpPspManager::ConnectPsp(self, peer, peer_control, psp_token_xchg_);
...
TEST_P(TcpPspManagerTest, ConnectBeforeAccept) {
  if (!psp::IsPspSupported()) {
    GTEST_SKIP() << "psp not supported";
  }

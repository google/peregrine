...
class TransportTest : public ::testing::TestWithParam<Param> {
 ...
 protected:
  ...
  bool WaitForCompletion(Transport& t, const Handle h) {
    if (completion_callback_) {
      return done_.WaitForNotificationWithTimeout(absl::Seconds(10));
    }
    while (true) {
      if (const absl::StatusOr<Status> s = t.Poll(h); IsCompleted(*s)) {
        completed_status_ = *s;
        return true;
      }
...
INSTANTIATE_TEST_SUITE_P(, TransportTest,
                         /*completion_callback=*/Values(false, true), ToString);
...
TEST_P(TransportTest, Read) {
  ...
  ASSERT_TRUE(WaitForCompletion(lt, *h));
  EXPECT_EQ(completed_status_, Status::kSuccess);
